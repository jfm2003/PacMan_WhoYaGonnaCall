...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(GHOST0, OBS0), (GHOST1, OBS1), (PAC, OBS0), (PAC, OBS1)]
    variableDomainsDict = {}

    legalPositions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ...
    maxTrueDistance = 0
    for p1 in legalPositions:
        for p2 in legalPositions:
            dist = manhattanDistance(p1, p2)
            if dist > maxTrueDistance:
                maxTrueDistance = dist

    maxObservedDistance = maxTrueDistance + MAX_NOISE
    obsValues = list(range(maxObservedDistance + 1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            factorsToJoin = [f for f in factors if var in f.variables()]
            if not factorsToJoin:
                continue

            factors = [f for f in factors if var not in f.variables()]

            remainingFactors, joinedFactor = joinFactorsByVariable(factorsToJoin, var)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        pacmanToGhost = manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(noisyDistance, pacmanToGhost)
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            probability = self.getObservationProb(observation, pacmanPos, position, jailPos)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            position = self.legalPositions[i % legal]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.particles:
            newDist = self.getPositionDistribution(gameState, oldPosition)
            particle = newDist.sample()
